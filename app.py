...
def laad_assets():
    ...
    # Update de globale variabelen direct vanuit de settings
    SPONSOR_ROTEREND = theme.get('geselecteerde_sponsors', [])
...
def control_panel(wedstrijd_id):
    ...
    load_players() # <--- TOEVOEGEN
    # Variabele 'spelers' doorgeven
    return render_template('control.html', wedstrijd=wedstrijd, spelers=SPELERS_DB)

@app.route('/display')
def display_page():
    # Haal altijd de meest recente data op
    instellingen = load_settings()
    theme = {**DEFAULT_SETTINGS, **instellingen.get('theme', {})}
    
    # We halen de specifieke waarden direct uit de zojuist geladen 'theme' dict
    huidige_sponsors = theme.get('geselecteerde_sponsors', [])
    huidige_logo = theme.get('standaard_thuis_logo')

    return render_template('display.html', 
        sponsor_lijst=huidige_sponsors,
        standaard_thuis_logo=huidige_logo,
        theme_json=theme
    )
...
try:
    ...
    laad_assets()
    get_local_ip()
    logging.info("App initialisatie voltooid: Wedstrijden, spelers en assets geladen.")
except Exception as e:
    logging.error(f"Fout tijdens initiële load: {e}")